...
그래서 지금 님은 바로 반응하기보다, <span class="crt_gold">다음 말을 더 정확하게 고르려 합니다.</span><br><br>
...
그렇다면 앞으로는 <span class="crt_gold">상대 말의 속도보다 내용부터 먼저 잡아 보세요.</span><br><br>

님은 감정에 휩쓸리는 사람이 아니라, 확인하고 움직이는 사람입니다.
서운한 말을 들었을 때 바로 맞받아치지 말고, 적어 두었다가 다시 말해 보세요.
<span class="crt_gold">급한 대답보다 다시 확인하는 습관을 살리세요</span><br><br>
`
    },
    {
    ...
    sajuExplains.forEach((item, index) => {
        ...
        card.innerHTML = `
            <div class="result_explainCard_titleBox" onclick="toggleExplain(${index}, this)">
                <p class="result_explainCard_title crt_pointGradient">${formatTitleWithBreak(item.title)}</p>
